...
class VivoStore(AppStoreBase):
    """vivo 开放平台"""

    store_type = AppStore.VIVO

    def __init__(self):
        super().__init__()
        self.config = config.vivo
    ...
    async def upload_app(
        ...
        logger.info(f"Uploading {file_path} to vivo Store...")
        return UploadResult(
            success=False,
            store=AppStore.VIVO,
            status=UploadStatus.FAILED,
            message="vivo Store not implemented yet",
        )
