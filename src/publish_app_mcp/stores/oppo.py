...
class OppoStore(AppStoreBase):
    """OPPO 开放平台"""

    store_type = AppStore.OPPO

    def __init__(self):
        super().__init__()
        self.config = config.oppo
    ...
    async def upload_app(
        ...
        logger.info(f"Uploading {file_path} to OPPO Store...")
        return UploadResult(
            success=False,
            store=AppStore.OPPO,
            status=UploadStatus.FAILED,
            message="OPPO Store not implemented yet",
        )
